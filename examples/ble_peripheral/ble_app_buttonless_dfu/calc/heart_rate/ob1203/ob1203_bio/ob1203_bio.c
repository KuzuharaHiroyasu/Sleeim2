...
#define OB1203_BIO_FIFO_SAMPLES_MAX_NUM_32 (32)
#define OB1203_BIO_MAX_NUMBER_OF_SAMPLES_20 (20)
#define OB1203_BIO_PPG2_MODE_SAMPLES_MASK_0XFE (0xFE)
...
bool ob1203_bio_ppg_raw_data_read(ob1203_bio_t *const p_bio, rm_ob1203_raw_data_t *const p_raw_data)
{
    ...
    if (RM_OB1203_PPG_INTERRUPT_TYPE_FIFO_AFULL == p_bio->interrupt_cfg.ppg_type)
    {
        ...
        number_of_samples = fifo_info.write_index;
        if (fifo_info.write_index < fifo_info.read_index)
        {
            /* Number of samples is up to 32. */
            number_of_samples += OB1203_BIO_FIFO_SAMPLES_MAX_NUM_32;
        }
        number_of_samples -= fifo_info.read_index;

        /* In PPG2 mode, Two samples is one pair. */
        number_of_samples &= OB1203_BIO_PPG2_MODE_SAMPLES_MASK_0XFE;

        /* Set overflow counter */
        p_bio->overflow = fifo_info.overflow_counter;

        /* Set number of samples */
        if (OB1203_BIO_MAX_NUMBER_OF_SAMPLES_20 < number_of_samples)
        {
            number_of_samples = OB1203_BIO_MAX_NUMBER_OF_SAMPLES_20;
        }
        p_bio->number_of_samples = number_of_samples;
    }
    else
    {
        /* Set overflow counter */
